# TODO: Engine errors are mapped to HTTP 400 via EngineCallError
from fastapi import APIRouter, HTTPException
from pydantic import BaseModel
from api.kafka.engine_calls.derivative import call_derivative
from api.kafka.client import EngineCallError

router = APIRouter()

class DerivativeIn(BaseModel):
    expr: str
    var: str = "x"

class DerivativeOut(BaseModel):
    raw: str  


@router.post("/derivative", response_model=DerivativeOut)
async def derivative_endpoint(body: DerivativeIn) -> DerivativeOut:
    try:
        data = await call_derivative(expr=body.expr, var=body.var)
    except EngineCallError as e:
        # engine answered with ok=False
        raise HTTPException(
            status_code=400,
            detail=f"engine error: {e}",
        )
    except HTTPException:
        # re-raise any HTTPException we already raised
        raise
    except Exception as e:
        # bad user input / unexpected error
        raise HTTPException(
            status_code=400,
            detail=f"invalid input: {e}",
        )
    
    return DerivativeOut(raw=data.get("raw", ""))
